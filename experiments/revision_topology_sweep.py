...
def graph_metrics(network) -> dict:
    """Compute structural graph statistics for a resistor network."""
...
def run_one(
    ...
    """Run one A -> B sequential-learning experiment for one topology and seed."""

    # Parameters chosen to give broadly comparable edge counts across topologies.
    # You can tune these later after inspecting num_edges.
    network = generate_connected_network(
        num_nodes=num_nodes,
        topology=topology,
        seed=seed,
        edge_prob=0.08,          # ER: expected mean degree ~ p(N-1) ≈ 6.3
        small_world_k=6,         # WS: mean degree = 6
        small_world_beta=0.2,
        scale_free_m=3,          # BA: mean degree ≈ 2m = 6
        geometric_radius=None,   # use default connected-friendly radius
        input_nodes=(0, 1),
...
def main() -> None:
    ...
    # For a quick test, change this to range(2).
    # For the actual revision, use range(20).
    seeds = range(20)
    ...
    out_path = "results/revision_topology_sweep.csv"
    ...
    summary_path = "results/revision_topology_sweep_summary.csv"
